...
public:
	...
	bool																						Run_MypathProcess_OnlyUIShow( CPString szName, CPString szParameters );
	bool																						Run_Process_OnlyUIShow( LPWSTR lpwzAppPath, LPWSTR lpwzCommandPath );
	...
	string																						GetSelfPCName();
	int																							GetSelfPhygicalAddress( vector<NetworkAdapterInfor> &OUT_vecRetPhygicalAddress, bool IN_bAllAdapterSearch );
	...
	tm																							Convert_mSec2Tm( unsigned long long ullTime, int &nMilliSeconds );
	unsigned long long																			Convert_CurrentTime2MilliSeconds();
	time_t																						GetNowTime( tm *pStTm );
	void																						GetTimeT2Tm( tm *pOUT_StTm, time_t tmIN_Time );
	void																						USleep( __int64 uSec );
	int																							CalTimeInfor_DateTime2HourCnt( time_t tmDateTime );
	tm*																							CalTimeInfor_HourCnt2DateTime( int nTime_HourCount );

protected:
	void																						ClearLocker();
	CPLocker*																					GetLockerInfor( unsigned int unLockIndex );
	unsigned long																				SetHiTimerResol( unsigned long timer_res_us );
...
protected:
	...
	unsigned long					m_ulCurrentResolution;
};
